...
class SMPLXTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb, dataloader_nb=0):
        ...
        with torch.no_grad():
            body_pred,_,_,_,_ = self.body_model(images, bbox_center=bbox_center, bbox_scale=bbox_scale, img_w=img_w, img_h=img_h, fl=fl)
    ...
    def validation_step(self, batch, batch_nb, dataloader_nb=0, vis=False, save=True, mesh_save_dir=None):
        ...
        body_pred,_,_,_,_  = self.body_model(images, bbox_center=bbox_center, bbox_scale=bbox_scale, img_w=img_w, img_h=img_h)
