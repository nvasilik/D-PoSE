...
class SMPLXTrainer(pl.LightningModule):
    def __init__(self, hparams, config_tune=None):
        ...
        self.body_model = HMR(
            ...
            pretrained_ckpt=self.hparams.TRAINING.PRETRAINED_CKPT_BODY,
            hparams=self.hparams,
        )
        self.hand_model = Hand(
            backbone=self.hparams.MODEL.BACKBONE,
            img_res=self.hparams.DATASET.IMG_RES,
            pretrained_ckpt=self.hparams.TRAINING.PRETRAINED_CKPT_HANDS,
            hparams=self.hparams,
        )
...
class Tester:
    ...
    def _load_pretrained_model(self):
        ...
        body_ckpt = torch.load(self.args.body_ckpt)['state_dict']
        
        self.model.body_model.load_state_dict(strip_prefix_if_present(body_ckpt, prefix='model'), strict=False)
        ...
        body_ckpt = torch.load(self.args.body_ckpt)['state_dict']
        body=[]
        for k in body_ckpt.keys():
            if 'body_model' in k:
                body.append(k)
        full_body=[]
        #import ipdb; ipdb.set_trace()
        for k in hands_ckpt.keys():
            if 'fullbody_model' in k or 'smplx' in k or 'smpl' in k:
                full_body.append(k)
        new_ckpt = {}
        new_ckpt.update({k:v for k,v in hands_ckpt.items() if k in hands})
        new_ckpt.update({k:v for k,v in body_ckpt.items() if k in body})
        import ipdb; ipdb.set_trace()
        #new_ckpt.update({k:v for k,v in hands_ckpt.items() if k in full_body})
        #import ipdb; ipdb.set_trace()
        #ckpt = torch.load(self.args.ckpt)['state_dict']
        '''
        #load_pretrained_model(self.model, new_ckpt, overwrite_shape_mismatch=True, remove_lightning=True)
        logger.info(f'Loaded pretrained weights from both {self.args.hands_ckpt} and {self.args.body_ckpt}')
    ...
    @torch.no_grad()
    def _run_on_image_folder(self, all_image_folder, detections, output_folder, visualize_proj=True):
            for fold_idx, image_folder in enumerate(all_image_folder):
                ...
                for img_idx, img_fname in tqdm.tqdm(enumerate(image_file_names)):
                    ...
                    lhand_joints = hmr_output['joints2d'][:, 25:40]
                    rhand_joints = hmr_output['joints2d'][:, 40:55]

                    center_r, scale_r = get_bbox_valid(rhand_joints, img_h, img_w, SCALE_FACTOR_HAND_BBOX)

                    #rgb_img_full = torch.tensor(img).cuda().float()
                    #rgb_img_full = rgb_img_full.unsqueeze(0).expand(inp_images.shape[0],-1,-1,-1)
                    right_hand_crop, _ = crop_tensor(img, center_r, scale_r, 224)
                    ...
                    full_body_pred = self.model.fullbody_model(hmr_output['body_feat'], left_hand_pred['hand_feat'], right_hand_pred['hand_feat'], hmr_output['pred_pose'], hmr_output['pred_shape'], hmr_output['pred_cam'],left_hand_pred['pred_pose'], right_hand_pred['pred_pose'],bbox_center, bbox_scale, img_w, img_h)

                    focal_length = (img_w * img_w + img_h * img_h) ** 0.5
                    pred_vertices_array = (full_body_pred['vertices'] + full_body_pred['pred_cam_t'].unsqueeze(1)).detach().cpu().numpy()
    ...
    def run_on_image_folder(self, all_image_folder, detections, output_folder, visualize_proj=False, save_result=False, eval_dataset=''):
        ...
        for fold_idx, image_folder in enumerate(all_image_folder):
            ...
            for img_idx, img_fname in tqdm.tqdm(enumerate(image_file_names)):
                ...
                body_pred,orig_depth,_,_,segmentation = self.model.body_model(inp_images, bbox_center=bbox_center, bbox_scale=bbox_scale, img_w=img_w, img_h=img_h)
